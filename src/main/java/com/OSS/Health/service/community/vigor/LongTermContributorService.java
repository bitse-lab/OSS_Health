...
@Service
public class LongTermContributorService {
    ...
    public void generateMonthlyReport() throws Exception {
        ...
        //清除表中的数据
        longTermContributorMapper.clearLongTermContributor();
        ...
        if (startDate.isBefore(currentDate)) {
            ...
            while (!dateToCheck.isAfter(currentDate)) {
                ...
                // 输出每月的长期贡献者数量
                LongTermContributor entity = new LongTermContributor();
                entity.setTime(dateToCheck);
                entity.setNumber(longTermContributors);
                // 使用 Mapper 插入数据
                longTermContributorMapper.insertLongTermContributor(entity);
