...
@Service
public class MonthVolunteerCommitsService_new{
	@Autowired
    private MysqlDataMapper mysqlDataMapper;

    private static final String MYSQL_ID = "2.1.4";	// 请替换为对应ID
	...
	public void generateMonthlyReport() throws Exception {
	    ...
	    try (Git git = Git.open(new File(repoPath))) {
	        ...
	        // 按月份统计 volunteerCommit 数量
	        Map<YearMonth, Integer> monthlypublicEmailCount = new HashMap<>();
	        ...
	        for (CommitInfo commitInfo : commitDataList) {
	            if (!commitInfo.isPrivateEmail()) {
	                ...
	                monthlypublicEmailCount.put(ym, monthlypublicEmailCount.getOrDefault(ym, 0) + 1);
	            }
	        }

	        // 每月的邮箱统计
	        for (Map.Entry<YearMonth, Integer> entry : monthlypublicEmailCount.entrySet()) {
