...
@Service
public class MonthForkService_new{
	@Autowired
    private MysqlDataMapper mysqlDataMapper;

    private static final String MYSQL_ID = "3.2.2";	// 请替换为对应ID
	...
	public void generateMonthlyReport() throws Exception {
        ...
        try(Git git = Git.open(new File(repoPath))){
	        ...
	        // 存储forkData
	        List<LocalDate> forkDatas= getForkData();
	        ...
	        if (startDate.isBefore(currentDate)) {	        	            
	            ...
	            while (!dateToCheck.isAfter(currentDate)) {
	            	...
	            	// 计算从dateToStartTmp到dateToCheckTmp 这30天内的新增fork数量
	            	int forkCount = 0;
	            	for (LocalDate forkData : forkDatas) {
	            	    if (!forkData.isBefore(dateToStartTmp) && !forkData.isAfter(dateToCheckTmp)) {
	            	        forkCount++;
	            	    }
	            	}
	                
	            	// 输出每月的fork数	                             
	                entity.setNumber(forkCount);
