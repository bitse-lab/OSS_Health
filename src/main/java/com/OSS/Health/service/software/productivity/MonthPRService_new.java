...
@Service
public class MonthPRService_new{
	@Autowired
    private MysqlDataMapper mysqlDataMapper;

    private static final String MYSQL_ID = "1.3.4";	// 请替换为对应ID
	...
	private Map<YearMonth, Integer> getPRData() throws IOException {
	    ...
	    for (JsonNode prNode : rootNode) {
	        ...
	        // 累加每月 PR 数量
	        monthlyPRCount.put(yearMonth, monthlyPRCount.getOrDefault(yearMonth, 0) + 1);
	    }	
	
	    return monthlyPRCount;
	}	
